...
export function validateRun(s){
 ...
 for(const rooms of s.floors){
  ...
  for(const r of rooms){check(Number.isInteger(r.x)&&Number.isInteger(r.y)&&Math.abs(r.x)<12&&Math.abs(r.y)<12&&roomTypes.has(r.type));check(!coords.has(`${r.x},${r.y}`));coords.add(`${r.x},${r.y}`);check(typeof r.seen==='boolean'&&typeof r.used==='boolean');
   ...
   if(r.shrineState!==undefined){check(r.type==='shrine'&&['sealed','active','choice','done'].includes(r.shrineState));if(r.shrineState==='done')check(r.used);if(r.shrineState==='choice')check(!r.used&&r.enemies.length===0&&r.incantations?.length===2);}
 ...
 if(p.relics!==undefined)check(Array.isArray(p.relics)&&p.relics.length<=31&&new Set(p.relics).size===p.relics.length&&p.relics.every(id=>relicInfo(id)));if(p.ultimateEssences!==undefined)check(Number.isInteger(p.ultimateEssences)&&number(p.ultimateEssences,0,10000));
