...
export function migrateEnemies(s){for(const [tier,rooms] of s.floors.entries())for(const r of rooms)for(const e of r.enemies){strengthenEnemy(e);if(tier>0)e.tier??=tier;if(s.key&&e.type!=='boss')e.escapeDepth??=s.floors.length-tier;if(e.type==='boss'&&!e.bossHealthVersion&&e.variant!=='slime'){const ratio=e.hp/e.max;e.max=Math.ceil(e.max*1.5);e.hp=e.hp<=0?0:Math.ceil(e.max*ratio);e.bossHealthVersion=1;}}return s;}
