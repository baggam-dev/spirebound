export function createInput(stick,onAction){
 ...
 function move(e){const b=stick.getBoundingClientRect(),radius=origin?36:b.width*.35,x=(e.clientX-(origin?.x??b.left+b.width/2))/radius,y=(e.clientY-(origin?.y??b.top+b.height/2))/radius,n=Math.max(1,Math.hypot(x,y));axis={x:x/n,y:y/n};stick.firstElementChild.style.transform=`translate(${axis.x*radius}px,${axis.y*radius}px)`;}
 stick.addEventListener('pointerdown',e=>{if(!enabled||pointer!==null)return;e.preventDefault();pointer=e.pointerId;if(window.matchMedia('(pointer:coarse)').matches){const b=stick.getBoundingClientRect();origin={x:e.clientX,y:e.clientY};stick.style.setProperty('--stick-x',`${e.clientX-b.left}px`);stick.style.setProperty('--stick-y',`${e.clientY-b.top}px`);stick.classList.add('active');}stick.setPointerCapture(pointer);move(e);});stick.addEventListener('pointermove',e=>{if(enabled&&pointer===e.pointerId)move(e);});for(const event of ['pointerup','pointercancel','lostpointercapture'])stick.addEventListener(event,e=>{if(pointer===e.pointerId)reset();});
 document.addEventListener('visibilitychange',()=>{if(document.hidden)reset();});window.addEventListener('resize',reset);
