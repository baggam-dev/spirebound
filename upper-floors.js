...
export const fanOffsets=e=>e.variant==='king'?(kingPhase(e)>=2?[-.56,-.28,0,.28,.56]:[-.35,0,.35]):e.type==='astralSniper'?(e.gateTitle?[-.36,-.18,0,.18,.36]:[-.18,0,.18]):[-.42,0,.42];
...
function resolve(e,p,r,dt,bullets,hurt){const a=e.darkAttack;if(!a)return false;
 ...
 if(a.kind==='fan')for(const offset of fanOffsets(e))shot(e,a.aim+offset,bullets,e.type==='astralSniper'?330:290);
...
export function tickUpperState(e,p,r,dt,bullets){
 ...
 if(phase>=2){e.guardClock=Math.max(0,(e.guardClock??1.5)-dt);if(e.guardPortal){e.guardPortal.time=Math.max(0,e.guardPortal.time-dt);if(!e.guardPortal.time){const living=r.enemies.filter(n=>n.summoned&&n.summoner===e.id&&n.hp>0).length;if(living<2){const id=Math.max(r.nextEnemyId||0,...r.enemies.map(n=>n.id+1));r.nextEnemyId=id+1;const child={id,type:'royalGuard',x:e.guardPortal.x,y:e.guardPortal.y,hp:180,max:180,cd:1.2,tier:7,balanceVersion:1,summoned:true,summoner:e.id,spawnGrace:1};safeSpawn(child,r.obstacles,22);r.enemies.push(child);}delete e.guardPortal;}}
...
export function updateUpper(e,p,r,dt,bullets,hurt){
 ...
 if(['starKnight','royalGuard','riftHunter'].includes(e.type))walk(e,p,r,dt,(e.gateTitle?88:78)*slowed);
