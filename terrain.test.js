...
test('enemies can route around cover to player',()=>{const obs=[{x:300,y:160,w:80,h:90}],p={x:240,y:205},target={x:450,y:205};for(let i=0;i<400;i++){const a=steering(p,target,obs);moveBody(p,Math.cos(a)*2,Math.sin(a)*2,obs,18);assert.ok(!blocked(p.x,p.y,18,obs));if(Math.hypot(p.x-target.x,p.y-target.y)<5)break;}assert.ok(Math.hypot(p.x-target.x,p.y-target.y)<5);});
