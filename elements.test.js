...
test('fire activates only on first contact; explicitly spent fire procs stay spent',()=>{
...
test('legacy fire investments, evolution and pending offers become poison exactly once',()=>{
 const s=newRun(8);delete s.skillSystemVersion;s.generationVersion=12;delete s.player.poison;s.player.fire=3;s.player.frost=2;s.player.evolutions={fire:'flare'};s.choices=['fire','haste','split'];const map=JSON.stringify(s.floors);const migrated=parseSave(encodeSave(s));assert.equal(migrated.player.fire,0);assert.equal(migrated.player.poison,3);assert.equal(migrated.player.evolutions.poison,'flare');assert.equal(migrated.choices,null);assert.equal(ultimateUnlocked(migrated.player),false);assert.equal(JSON.stringify(migrated.floors),map);migrated.player.fire=1;assert.equal(parseSave(encodeSave(migrated)).player.fire,1);
});
...
test('poison arrows infect every pierced target, respect penetration count and do not hit twice',()=>{
 for(const scale of [1,.45,.6]){const s=newRun(4),r=currentRoom(s);s.player.poison=1;s.attack=999;r.obstacles=[];r.enemies=[enemy(550,300,1000,0),enemy(650,300,1000,1),enemy(750,300,1000,2)];s.projectiles=[{x:500,y:300,vx:420,vy:0,enemy:false,elemental:true,damageScale:scale,life:3,pierce:1,hit:[]}];stepRun(s,.65);assert.equal(r.enemies[0].poisonStacks.length,1);assert.equal(r.enemies[1].poisonStacks.length,1);assert.equal(r.enemies[2].poisonStacks,undefined);assert.equal(r.enemies[1].poisonStacks[0].dps,4.8*scale);stepRun(s,.01);assert.equal(r.enemies[0].poisonStacks.length,1);assert.equal(r.enemies[1].poisonStacks.length,1);}
});
test('poison piercing continues after saving an arrow whose first impact is spent',()=>{
 const s=newRun(4),r=currentRoom(s);s.player.mainSkill='poison';s.player.poison=1;s.attack=999;r.obstacles=[];r.enemies=[enemy(550,300,1000,0),enemy(750,300,1000,1)];s.projectiles=[{x:500,y:300,vx:420,vy:0,enemy:false,elemental:true,damageScale:1,life:3,pierce:1,hit:[]}];stepRun(s,.2);assert.equal(r.enemies[1].poisonStacks,undefined);const copy=parseSave(encodeSave(s));stepRun(copy,.5);assert.equal(currentRoom(copy).enemies[1].poisonStacks.length,1);assert.equal(currentRoom(copy).enemies[0].poisonStacks.length,1);
});
