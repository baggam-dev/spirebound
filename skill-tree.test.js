...
test('frost freezes on exactly three primary impacts and cannot permanently lock enemies',()=>{
 for(const type of ['archer','boss']){const e=enemy(0,600,300,type),p={damage:1,frost:1};hitEnemy(p,e,[e]);hitEnemy(p,e,[e]);assert.ok(!e.frozen);hitEnemy(p,e,[e]);assert.equal(e.frozen,type==='boss'?.45:1.2);for(let i=0;i<8;i++)hitEnemy(p,e,[e]);assert.equal(e.frostStacks,0);tickEffects(e,1.3);assert.equal(e.frozen,0);hitEnemy(p,e,[e]);assert.equal(e.frostStacks,0);tickEffects(e,2);hitEnemy(p,e,[e]);assert.equal(e.frostStacks,1);tickEffects(e,3.1);assert.equal(e.frostStacks,0);}
});
...
test('aura hits nearby grounded enemies only and damage support contributes to ultimate unlock',()=>{
 const s=newRun(1),r=currentRoom(s);s.player.aura=2;s.attack=999;r.enemies=[enemy(0,520),enemy(1,700)];stepRun(s,.5);assert.equal(r.enemies[0].hp,992);assert.equal(r.enemies[1].hp,1000);assert.equal(s.metrics.damageDealt,8);const e=enemy();hitEnemy({damage:20,power:5},e,[e]);assert.equal(e.hp,974);assert.ok(ultimateUnlocked({fire:1,power:2,aura:1,repeat:1}));
});
...
test('fire splash and field radii are halved and all fire damage is reduced fifteen percent',()=>{
 const p={damage:20,fire:3},e=enemy(),r={enemies:[e]};hitEnemy(p,e,r.enemies,[],1,true,r);assert.ok(Math.abs(e.hp-(1000-20-21.4*.85))<1e-9);assert.equal(r.fireZones?.length||0,0);assert.equal(e.burnField.r,42.5);assert.equal(e.burnField.dps,10.2);assert.equal(e.burnStacks[0].dps,4.76);
});
