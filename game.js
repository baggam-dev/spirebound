...
function title(){
 ...
 panel(`<small>A JOURNEY UP. A FIGHT BACK.</small><h2>SPIREBOUND</h2><p>왕실 정찰병 · 8층 왕복 · ${RELEASE}</p><p>열쇠를 빼앗고 살아 돌아오세요.<br>기본 공격은 자동, 궁극기는 5레벨부터 선택할 수 있습니다.</p>${exists?`<p>저장된 도전: ${loaded.run.floor+1}/${loaded.run.floors.length}층 · ${timeString(loaded.run.elapsed)}${loaded.recovered?' · 백업 복구 가능':''}</p><button class="primary" id="continue">이어하기</button>`:''}${loaded.error?`<p role="alert">${escapeHtml(loaded.error)}</p>`:''}<button ${exists?'':'class="primary"'} id="start">새 도전 →</button><div><button id="guide">조작 안내</button><button id="records">도전 기록</button><button id="bossPractice">보스 테스트</button></div>`);
...
function levelUp(){
 ...
 if(!choices.length){s.player.hp=Math.min(s.player.max,s.player.hp+1);consumeLevelChoice(s);resume();return;}
 panel('<small>CHOOSE YOUR PATH</small><h2>새로운 깨달음</h2><p>'+ (choices[0]?.growth?'기술 강화를 마쳤습니다. 이번 도전의 기본 능력을 추가로 성장시키세요.':'궁극기는 5·10레벨 미습득 시 확정 후보, 그 외 5레벨 이후 30% 확률로 등장합니다.')+'</p><div class="cards">'+choices.map((k,i)=>'<button class="skill-'+k.grade+'" id="u'+i+'">'+iconSVG(k.icon||k.id)+'<b>'+ '['+(k.growth?'추가 성장':gradeLabels[k.grade])+'] '+k.name+(k.growth?'':' '+((s.player[k.id]||0)+1)+' / '+k.max)+'</b><span>'+k.description+'</span><em>'+recipes.filter(r=>r.needs.includes(k.id)&&r.needs.every(id=>allowedSkill(s.player,id))).map(r=>r.name+' · '+r.needs.map(id=>skills.find(a=>a.id===id).name).join(' + ')).join('<br>')+'</em></button>').join('')+'</div><button id="reroll" '+((s.rerolls??1)>0&&!choices.every(k=>k.growth)?'':'disabled')+'>후보 새로고침 · '+(s.rerolls??1)+'회</button>');
...
function skill(){if(paused||!castUltimate(s))return;ensureMetrics(s).ultimatesUsed++;toast(s.player.evolutions?.ultimate==='turret'?'궁극기 · 자동 저격 석궁':'궁극기 · 화살비');}
function interaction(){
 if(s?.practice)return null;
 if(!s||s.status!=='playing')return null;const r=room();
 if(r.hasChest&&!r.chestUsed&&!r.enemies.length&&distance(s.player,{x:600,y:115})<=65)return ['보물 상자 열기',()=>{prepareChest(s);save();if(openChest(s)){save();toast(lootLabel(room().lastLoot));resume();}}];
 if(distance(s.player,{x:480,y:115})>78)return null;
 if(r.kingPending)return ['타락한 왕에게 도전',()=>{if(challengeKing(s)){save();toast('타락한 왕, 모르드가 왕좌에서 일어납니다.');}}];
 if(r.type==='fountain'&&!r.used&&!r.enemies.length)return [s.player.hp>=s.player.max?'샘물 · 체력이 가득합니다':'샘물로 회복',()=>{if(!useFountain(s))return;save();toast('샘물이 말랐습니다. 3하트 회복');}];
 ...
 if(r.type==='down'&&!roomLocked(s))return [s.floor+'층으로 내려가기',()=>changeFloor(s.floor-1)];
 if(r.type==='shrine'&&!r.used&&!r.enemies.length)return ['고대인의 주술 · 반드시 하나 선택',shrine];
 if(canEscape(s))return ['열쇠로 탈출하기',()=>finish(true)];
 if(r.type==='exit')return ['출구 봉쇄 · 열쇠 필요',()=>toast('최상층 수호자가 열쇠를 지키고 있습니다.')];return null;
}
function shrine(){if(!prepareIncantations(s))return;stop('shrine');save();const r=room();panel('<small>ANCIENT INCANTATION</small><h2>고대인의 주술</h2><p>문이 봉인되었습니다. 두 주술 중 하나를 반드시 받아들여야 합니다.<br>선택 결과는 이번 도전 동안 유지됩니다.</p><div class="cards">'+r.incantations.map((id,i)=>{const k=incantationInfo(id);return '<button id="hex'+i+'" class="incantation '+(k.good?'blessing':'curse')+'">'+iconSVG(k.good?'sun':'poison')+'<b>'+k.name+'</b><span>'+(k.good?'축복':'저주')+' · '+k.description+'</span></button>';}).join('')+'</div>');r.incantations.forEach((id,i)=>button('hex'+i,()=>{if(useShrine(s,i)){save();resume();}}));}
...
function update(dt){if(!s||paused)return;const beforeWarning=heavyWarningKey();const result=stepRun(s,dt,input.movement());bullets=s.projectiles;if(heavyWarningKey()&&heavyWarningKey()!==beforeWarning)playHeavyWarning();fx.push(...result.effects);fx=fx.filter(f=>(f.t-=dt)>0);
 ...
 const messages={gateReward:'정예 관문 돌파 · 계단에서 유물을 선택하세요.',eliteReward:'정예 처치 · 물약 1개 획득',trialReward:'시련 완료 · 제단에서 보상을 선택하세요.',shield:'보호막이 공격을 막았습니다.',potion:'회복 물약 획득',key:'열쇠 획득! 1층 출구로 귀환하세요.',stairs:'중간 보스 격파! 위층 계단이 열렸습니다.'};for(const event of result.events)if(messages[event])toast(messages[event]);
...
function sprite(x,y,type){ctx.save();ctx.translate(Math.round(x),Math.round(y));rect(-13,13,27,7,'#0006');if(type==='player'){if(s?.player.aura>=3&&!(s.auraShield>0)){ctx.fillStyle='#ffffff55';ctx.fillRect(-13,-25,27,48);ctx.strokeStyle='#f5ffff';ctx.lineWidth=2;ctx.strokeRect(-13,-25,27,48);}rect(-10,-9,20,24,'#58785e');rect(-8,-21,16,15,'#aac0a0');rect(-6,-15,12,9,'#d5b48b');rect(-11,12,8,9,'#8f9e78');rect(4,12,8,9,'#8f9e78');rect(13,-10,4,30,'#c0a471');rect(17,-6,3,22,'#775f43');rect(-8,-23,16,5,'#446c56');}else if(type==='boss'){rect(-27,-24,54,51,s?.key?'#b84e56':'#706875');rect(-21,-37,42,29,s?.key?'#e8777a':'#928782');rect(-30,-45,10,24,'#bca57e');rect(20,-45,10,24,'#bca57e');rect(-14,-24,9,5,'#ff836c');rect(6,-24,9,5,'#ff836c');rect(-12,2,24,13,'#ab7358');rect(-36,0,9,35,'#bfa784');}else{let c=s?.key?'#b75553':type==='archer'?'#a08fbd':type==='charger'?'#b2976f':type==='scatter'?'#ce8e70':'#839985';rect(-11,-12,22,27,c);rect(-8,-20,16,15,c);rect(-6,-13,4,4,'#eddda9');rect(3,-13,4,4,'#eddda9');rect(-12,13,8,7,'#48574c');rect(4,13,8,7,'#48574c');}ctx.restore();}
function draw(){updateHUD(s);ctx.clearRect(0,0,960,540);rect(0,0,960,540,'#11191b');for(let y=40;y<510;y+=30)for(let x=25;x<940;x+=35){let n=((x*17+y*31)%19);rect(x,y,34,29,n<5?'#283231':n<12?'#252e2d':'#222c2b');if(n<3)rect(x+4,y+20,12,2,'#354039');}for(let x=0;x<960;x+=48){rect(x,0,46,30,'#3c4742');rect(x+2,31,43,13,'#202927');rect(x,508,46,32,'#3c4742');}for(let y=35;y<510;y+=40){rect(0,y,25,38,'#3e4943');rect(935,y,25,38,'#3e4943');}for(const x of [85,875])for(const y of [82,455]){rect(x-8,y,16,20,'#515b4d');rect(x-4,y-12,8,15,'#cf9854');rect(x-2,y-17,5,12,'#f0cc7c');}
 ...
 drawEssences(ctx,r,s.elapsed);drawElites(ctx,r);drawHazards(ctx,r);drawElements(ctx,r);drawOpenings(ctx,r);if(s.player.aura){ctx.strokeStyle='#b3dba855';ctx.beginPath();ctx.arc(s.player.x,s.player.y,auraProfile(s.player.aura).radius*(1+relicStat(s.player,'auraRange')),0,Math.PI*2);ctx.stroke();}
 if(s.player.aura>=3&&!(s.auraShield>0)){ctx.strokeStyle='#fff';ctx.lineWidth=2;ctx.beginPath();ctx.arc(s.player.x,s.player.y,27,0,Math.PI*2);ctx.stroke();}
 drawUpperLinks(ctx,r,s.elapsed);for(const e of r.enemies)drawUpperGround(ctx,e,s.elapsed);
 r.enemies.forEach(e=>{if(e.chargeAngle!==undefined&&e.cd>=chargeProfile(e).duration){ctx.strokeStyle='#e2b16e';ctx.setLineDash([7,5]);ctx.beginPath();ctx.moveTo(e.x,e.y);ctx.lineTo(e.x+Math.cos(e.chargeAngle)*120,e.y+Math.sin(e.chargeAngle)*120);ctx.stroke();ctx.setLineDash([]);}if(e.type==='ricochet'&&e.cd<.65){ctx.strokeStyle='#dd9973';ctx.beginPath();ctx.arc(e.x,e.y,e.type==='boss'?43:26,0,Math.PI*2);ctx.stroke();}if(e.variant!=='king'&&!upperTypes.includes(e.type)&&e.type!=='brute'&&e.type!=='flower'&&e.type!=='minislime'&&e.variant!=='slime'){ctx.save();ctx.translate(e.x,e.y);ctx.scale(e.scale??1,e.scale??1);ctx.translate(-e.x,-e.y);sprite(e.x,e.y,e.type);ctx.restore();}drawUpper(ctx,e,s.elapsed);drawReturnEnemy(ctx,e);drawEnemyDetails(ctx,s.key?{...e,escapeDepth:e.escapeDepth||1}:e);drawPoisonEnemy(ctx,s.key?{...e,escapeDepth:e.escapeDepth||1}:e);if(e.variant==='prism')drawPrism(ctx,s.key?{...e,escapeDepth:1}:e,r.obstacles,r.enemies);drawPattern(ctx,e);drawRanged(ctx,s.key?{...e,escapeDepth:e.escapeDepth||1}:e,r.obstacles);if(e.burn>0)rect(e.x-17,e.y-8,4,10,'#ed9a55');if(e.slow>0)rect(e.x+14,e.y-8,4,10,'#91d0e3');rect(e.x-15,e.y-30,30,3,'#131b1b');rect(e.x-15,e.y-30,30*Math.max(0,e.hp/e.max),3,s.key?'#d7766b':'#a7ad7b');});if(s.invulnerable<=0||Math.floor(s.invulnerable*20)%2===0)sprite(s.player.x,s.player.y,'player');bullets.filter(b=>!(b.delay>0)).forEach(b=>rect(b.x-3,b.y-2,b.frostShard?10*(b.shardSize||1):b.poisonShot==='orb'?12:b.enemy?7:10,b.frostShard?4*(b.shardSize||1):b.poisonShot==='orb'?12:4,b.passive?({sunFairy:'#ff993f',snowFairy:'#99ddff',stormFairy:'#ffe16a',turret:'#e9d2a4'})[b.passive]:b.poisonShot==='orb'?'#e5bdff':b.poisonShot==='needle'?'#c783f5':b.dark?'#d998ff':b.ricochet?'#74e1dd':b.enemy?'#e88373':b.element==='fire'?'#ff9948':b.element==='poison'?'#85ec79':b.element==='frost'?'#b4ecff':b.element==='chain'?'#bbacff':s.player.split&&s.player.pierce?'#8fe0cd':'#e6d392'));drawPassives(ctx,s);fx.forEach(f=>{if(f.thunder){drawThunder(ctx,f);return;}if(f.text){ctx.save();ctx.fillStyle=f.color;ctx.font='bold 20px sans-serif';ctx.textAlign='center';ctx.strokeStyle='#101016';ctx.lineWidth=4;ctx.strokeText(f.text,f.x,f.y-(1-f.t)*22);ctx.fillText(f.text,f.x,f.y-(1-f.t)*22);ctx.restore();return;}ctx.strokeStyle=f.color;ctx.lineWidth=3;ctx.beginPath();if(f.slash){ctx.moveTo(f.x-16,f.y+14);ctx.lineTo(f.x+16,f.y-14);ctx.moveTo(f.x+8,f.y-16);ctx.lineTo(f.x+18,f.y-18);}else if(f.toX!==undefined){ctx.moveTo(f.x,f.y);ctx.lineTo(f.toX,f.toY);}else ctx.arc(f.x,f.y,f.instant?f.r:f.r*(1-f.t),0,Math.PI*2);ctx.stroke();});
 drawRoomStory(ctx,s,r);drawMinimap(ctx,s.floors[s.floor],r,s.key?returnRoutes(s)[s.routePreference||'short']:[],mapped=>[0,1,2,3].map(d=>neighbor(s,d,s.floors[s.floor].indexOf(mapped))>=0));$('route').hidden=!s.key||s.status!=='playing';if(s.key){ctx.strokeStyle='#b74c4866';ctx.lineWidth=8;ctx.strokeRect(4,4,952,532);}const boss=r.enemies.find(e=>e.type==='boss');if(boss?.variant==='prism'){const left=boss.summonClock??PRISM_SUMMON.first,count=r.enemies.filter(e=>e.summoned&&e.summoner===boss.id).length;rect(350,43,260,5,'#252532');rect(350,43,260*Math.max(0,1-left/PRISM_SUMMON.interval),5,'#c6a1ef');text(count>=3?'소환 대기 · 정예 3/3':'정예 소환 '+left.toFixed(1)+'초 · '+count+'/3',480,61,'#d8b8ee',10);}if(boss){rect(280,27,400,7,'#182120');rect(280,27,400*bossHealth(r).ratio,7,'#b67864');if(boss.variant==='king'){for(const ratio of [.65,.3])rect(280+400*ratio,25,2,11,'#eed6f7');if(boss.guardPortal)text('망령 소환 중',480,53,'#dfbdff');}text(boss.variant==='king'?'타락한 왕, 모르드 · '+['','검은 왕','타락한 왕','왕관 속 마왕'][kingPhase(boss)]:boss.variant==='slime'?'독성 군체 · 남은 분열체 '+r.enemies.filter(e=>e.type==='boss').length:boss.variant==='prism'?'프리즘의 수호자':'녹슨 탑의 파수꾼',480,20,'#d6bfa1',11);}
 $('potion').disabled=paused||s.potionCooldown>0||!s.player.potions||s.player.hp>=s.player.max;$('health').setAttribute('aria-label',`체력 ${s.player.hp}/${s.player.max}하트`);$('floor').textContent=`${s.floor+1}층 · ${floorNames[s.floor]}`;$('phase').textContent=s.practice?'BOSS PRACTICE / 보스 테스트':s.status==='won'?'THE ESCAPE / 탈출 완료':s.status==='dead'?'THE END / 도전 종료':s.key?'THE FRENZY / 광폭 '+(s.floors.length-s.floor)+'단계':'THE ASCENT / 탐험';$('health').textContent=healthDisplay(s.player)+(s.player.armor>0?(s.shield>0?' · ◈ '+Math.ceil(s.shield)+'초':' · ◈ 준비'):'');$('level').textContent=`LV ${s.player.level} · XP ${s.player.xp}/${xpRequired(s.player.level)}`;$('timer').textContent=timeString(s.elapsed);$('objective').textContent=s.practice?(s.status==='playing'?'선택한 보스를 처치하세요 · ESC로 같은 빌드 재도전':'다시 도전하거나 다른 보스를 선택하세요.'):s.status==='won'?'탑의 봉인을 넘어 살아 돌아왔습니다.':s.status==='dead'?'원정 기록에서 빌드와 피격 원인을 확인하세요.':r.gate&&!r.used&&!s.key?'두 문지기를 처치하고 왕좌의 봉인을 해제하세요.':r.tutorial&&!s.key&&r.enemies.length?'첫 전투 · 추격자 4마리를 처치하고 첫 기술을 선택하세요.':s.key?'같은 길을 따라 1층 출구로 귀환하세요.':r.relicOffers&&!r.relicClaimed?'유물을 선택하거나 유물함을 지나가세요.':s.floor===s.floors.length-1?'최상층 보스를 처치하고 열쇠를 획득하세요.':s.floor%2?(s.floors[s.floor].some(r=>r.type==='boss'&&r.used)?'열린 계단을 찾아 위층으로 올라가세요.':'보스를 처치하고 위층 계단을 여세요.'):'위층 계단을 탐색하세요.';$('potion').textContent=s.potionCooldown>0?`물약 ${s.potionCooldown.toFixed(1)}초`:`물약 ${s.player.potions} · Q`;$('skill').hidden=mode==='title'||!ultimateUnlocked(s.player);$('skill').disabled=s.skill>0;$('dodge').disabled=s.dodge>0;$('skill').textContent=s.skill>0?`${s.player.evolutions?.ultimate==='turret'?'석궁':'화살비'} ${s.skill.toFixed(1)}s`:(s.player.evolutions?.ultimate==='turret'?'석궁 · F':'화살비 · F');$('dodge').textContent=s.dodge>0?`점멸 ${s.dodge.toFixed(1)}s`:'점멸 · SPACE';let i=interaction();$('interact').hidden=!i||paused;if(i)$('interact').textContent=i[0]+' · E';
}
