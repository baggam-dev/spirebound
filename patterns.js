...
export const bossPatterns=['ring','slam','fan','spiral','cross'];
export function emitShot(e,angle,speed,bullets){speed*=attackProfile(e).speed;bullets.push({x:e.x,y:e.y,vx:Math.cos(angle)*speed,vy:Math.sin(angle)*speed,enemy:true,source:enemyName(e)+(e.elite?' 정예탄':' 탄환'),damage:e.escapeDepth>=4?12:10,life:4,hit:[]});}
function recover(e,seconds){e.attackPhase='recover';e.attackTime=seconds*(e.type==='boss'?.5:ENEMY_COOLDOWN_FACTOR*attackProfile(e).recovery);}
export function updatePattern(e,p,obstacles,dt,bullets,enraged=false){
 ...
 if(e.attackPhase==='firing'&&e.attackTime<=0){
  ...
  if(e.pattern==='burst'){emitShot(e,e.patternAim,190,bullets);e.attackTime=.22;if(e.shot>=5+attackProfile(e).count)recover(e,enraged?1.7:2.4);}
 ...
 if(e.attackPhase==='recover'&&e.attackTime<=0){e.attackPhase=null;e.cd=e.type==='boss'?.3:.5*ENEMY_COOLDOWN_FACTOR;}
 return 0;
}
export function drawPattern(ctx,e){
 ...
 if(e.type==='boss'){ctx.setLineDash([]);ctx.font='11px sans-serif';ctx.textAlign='center';ctx.fillStyle=e.attackPhase==='recover'?'#a8d5b1':'#efd7a4';ctx.fillText(e.attackPhase==='recover'?'빈틈!':({ring:'원형 탄막',slam:'내려찍기',fan:'교차 부채꼴',spiral:'회전 탄막',cross:'회전 십자포화'})[e.pattern],e.x,e.y-57);}
 ctx.restore();
}
