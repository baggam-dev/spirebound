...
export function drawOpenings(ctx,room){ctx.save();ctx.textAlign='center';ctx.font='12px sans-serif';for(const e of room.enemies)if(e.opening>0){ctx.strokeStyle='#aee2b1';ctx.lineWidth=2;ctx.beginPath();ctx.arc(e.x,e.y,30,0,Math.PI*2);ctx.stroke();ctx.fillStyle='#d0f3c2';ctx.fillText(e.counterReason==='cover'?'충돌! 반격 기회':e.counterReason==='evade'?'연속 베기 회피! 반격':'빈틈!',e.x,e.y-48);}ctx.restore();}
