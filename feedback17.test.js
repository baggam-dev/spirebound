...
test('fire growth keeps level one and reduces later explosion increments',()=>{const loss=[];for(let fire=1;fire<=3;fire++){const e=foe(1000);elementalImpact({damage:20,fire},e,[e],{enemies:[e]});loss.push(1000-e.hp);}assert.ok(Math.abs(loss[0]-12.75)<1e-9);assert.ok(Math.abs(loss[2]-loss[1]-2.72)<1e-9);});
...
test('raw chest weights total one hundred and invalid rewards redraw only positives',()=>{
 assert.equal(lootTable.reduce((n,k)=>n+k.weight,0),100);const p=newRun(1).player;assert.equal(drawLoot(p,()=>0).id,'potion');assert.equal(drawLoot(p,()=>.5).id,'elites');assert.equal(drawLoot(p,()=>.98).id,'boss');for(const k of skills)p[k.id]=k.max;for(let i=0;i<100;i++){const result=drawLoot(p,()=>i/100,true);assert.ok(!lootTable.find(k=>k.id===result.id).trap);assert.ok(!result.skill);}
});
