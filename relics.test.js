...
import {offerRelics,claimRelic,directRelicFactor,movementSpeed,dodgeCooldown,ownedRelics} from './relics.js';
...
test('every boss offers deterministic unowned relics once including the final boss',()=>{
 const a=boss(),b=boss();bossDefeated(a);bossDefeated(b);assert.deepEqual(currentRoom(a).relicOffers,currentRoom(b).relicOffers);assert.equal(currentRoom(a).relicOffers.length,3);
 const before=currentRoom(a).relicOffers.slice();offerRelics(a,currentRoom(a));assert.deepEqual(currentRoom(a).relicOffers,before);
 a.floor=7;a.room=a.floors[7].findIndex(r=>r.type==='boss');currentRoom(a).enemies=[];assert.equal(bossDefeated(a),'key');assert.equal(currentRoom(a).relicOffers.length,3);
});
test('relics accumulate without duplicates and cannot be reclaimed after save',()=>{
 let s=boss();bossDefeated(s);const first=currentRoom(s).relicOffers[0];assert.ok(claimRelic(s,first));s=parseSave(encodeSave(s));assert.equal(claimRelic(s,first),false);
 s.floor=3;s.room=s.floors[3].findIndex(r=>r.type==='boss');currentRoom(s).enemies=[];bossDefeated(s);assert.ok(!currentRoom(s).relicOffers.includes(first));const second=currentRoom(s).relicOffers[0];assert.ok(claimRelic(s,second));assert.deepEqual(ownedRelics(s.player),[first,second]);
});
...
test('relic benefits use exact distance boundaries, shield limits and cooldowns',()=>{
 const p={relic:'lens',x:0,y:0};assert.equal(directRelicFactor(p,{x:139,y:0}),1);assert.equal(directRelicFactor(p,{x:140,y:0}),1);assert.equal(directRelicFactor(p,{x:280,y:0}),1.2);
 assert.equal(shieldCooldown({armor:2,relic:'iron'}),27);assert.equal(shieldCooldown({armor:99,relic:'iron'}),12);assert.equal(dodgeCooldown({relic:'iron'}),10);assert.equal(dodgeCooldown({}),10);
 const s=newRun();s.player.ultimate=1;s.player.evolutions={ultimate:'burst'};s.player.ultimate=1;s.player.evolutions={ultimate:'burst'};s.player.split=4;s.player.chain=1;s.player.relic='rain';assert.ok(castUltimate(s));assert.equal(s.skill,20);
});
